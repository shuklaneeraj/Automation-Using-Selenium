...
public class SFDC_Case_Updated_Persona {

	final static Logger log = LogManager.getLogger(SFDC_Case_Updated_Persona.class);
	...
	public void test(String SdEmID, String SdPwrd, String SSrlNum) throws Exception {
		...
		WebElement SerlNum = driver.findElement(By.xpath("//input[@id='pg:FormId:main:serialNo']"));
		JavascriptExecutor jse = (JavascriptExecutor) driver;
		jse.executeScript("arguments[0].value='SGH233ATKK';", SerlNum);
		Thread.sleep(4000);
		...
		WebElement prdNo = driver
				.findElement(By.xpath("//table[@class='list slds-table slds-table_striped']//tr[1]//td[1]//a[1]"));

		jse.executeScript("arguments[0].click();", prdNo);
		Thread.sleep(8000);
...
//		// Click on Ok Button
		 ...
		 * 
		 * System.out.println("Clicked On Done2 Button "); Thread.sleep(80000);
		 **/
		...
		gl.inputText(text, "text", "adidas AG");
		Thread.sleep(8000);

		By ctry = By.xpath("(//select[@class='slds-select'])[1]");
		gl.selectByText(ctry, "ctry", "Germany");
		Thread.sleep(8000);
		...
		jse.executeScript("arguments[0].click();", chk_R1);
		Thread.sleep(2000);
		...
		By chk = (By.xpath("(//span[@class='slds-radio_faux'])[24]"));
		...
		jse.executeScript("arguments[0].click();", chk_R2);
		Thread.sleep(2000);**/

//		By ok = By.xpath("//button[@title='OK']");
//		gl.clickButton(ok, "Clicked Ok");
//		Thread.sleep(60000);
		...
		/**
		 * boolean foundAlert = false; // WebDriverWait wait = new WebDriverWait(driver,
		 * 10); try { driver.switchTo().alert().accept();
		 * driver.switchTo().alert().accept(); foundAlert = true; } catch
		 * (NoAlertPresentException e) { foundAlert = false; }
		 * System.out.println(foundAlert); Thread.sleep(20000);
		 **/
